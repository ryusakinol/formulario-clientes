...
function construirTr(cliente) {
   ...
   clienteTr.classList.add("cliente")
    ...
    clienteTr.appendChild(construirTd(cliente.compañia, "info-compañia"));
...
function validarCliente(cliente){
    ...
    if (cliente.telefono.length == 0){
        errores.push("Telefono no puede estar vacio")
    } 
    ...
    if (cliente.compañia.length == 0){
        errores.push("Compañia no puede estar vacia")
    }
    return errores;
}
